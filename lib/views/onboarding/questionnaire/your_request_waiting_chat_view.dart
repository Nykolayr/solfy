...
class YourRequestWaitingChatView extends StatefulWidget {
  ...
  @override
  _YourRequestWaitingChatViewState createState() => _YourRequestWaitingChatViewState();
}

class _YourRequestWaitingChatViewState extends State<YourRequestWaitingChatView> {
          ...
          if (bloc.state is QuestionnaireFoundError) {
            Timer(Duration(seconds: 2), () {
              setState(() {
                isFinalTextVisible = true;
                isAnyError = true;
                errorMessage = (bloc.state as QuestionnaireFoundError).errors.errors![0].message;
                errorCode = (bloc.state as QuestionnaireFoundError).errors.errors![0].code;
              });
            });
            
            ModalHelpers.showError(context, (bloc.state as QuestionnaireFoundError).errors)
                .then((value) {
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        isSecondItemVisible
                            ? Column(
                                children: [
                                  ChatItem("making_a_request_to_the_traffic_police".tr(),
                                      radius: radiusMiddle),
                                  SizedBox(height: 4.h),
                                ],
                              )
                            : SizedBox(),
                        isStartLoadingVisible ? ChatLoadingItem(radius: radiusLast) : SizedBox(),
                        isThirdItemVisible
                            ? ChatItem(
                    ...
                    ? Column( 
                      children: [
                        Row(
                          ...
                          isAnyError ? 
                          SizedBox(height: 44,) : Container(),
                          isAnyError ? 
                          Padding(
                            padding: EdgeInsets.only(right: 16),
                            child: LongButtonWithText(text: "Понятно", onTap:() {
                              final bloc = context.read<QuestionnaireBloc>();
                              errorCode = (bloc.state as QuestionnaireFoundError).errors.errors![0].code;
                              if (errorCode == '907') {
                                context.router.replaceAll([ShortFormView()]);
                              }
